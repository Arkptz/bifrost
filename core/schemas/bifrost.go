...
const (
	...
	BifrostContextKeyAttemptBase                         BifrostContextKey = "bifrost-attempt-base"                   // int (base offset added to the inner retry loop's attempt index when stamping BifrostContextKeyNumberOfRetries; set by passthrough failover so each physical upstream call gets a unique governance billing identity — absent ⇒ 0, so non-passthrough routes are byte-identical (set by bifrost - DO NOT SET THIS MANUALLY))
	BifrostContextKeyPassthroughSingleAttempt            BifrostContextKey = "bifrost-passthrough-single-attempt"     // bool (when set, the inner retry loop in executeRequestWithRetries runs exactly one physical call so passthrough failover owns its own retry accounting and passthroughMaxTotalAttempts is an honest bound; absent ⇒ inner loop honors max_retries (set by bifrost - DO NOT SET THIS MANUALLY))
	BifrostContextKeyPassthroughRotatedKeys              BifrostContextKey = "bifrost-passthrough-rotated-keys"       // map[string]bool (key IDs already rejected 401/402/403 on an EARLIER leg of this passthrough failover. Seeds usedKeyIDs — a SOFT skip that the key provider resets once the pool is exhausted — never deadKeyIDs, which would shrink the eligible pool and let a pool 401 escalate cheap->expensive. Bounded by the provider's configured key pool (set by bifrost - DO NOT SET THIS MANUALLY))
	...
	BifrostContextKeyStreamConnHandle                    BifrostContextKey = "stream_conn_handle"             // interface{ SetReadDeadline(time.Time) error } (set by StreamPassthrough - DO NOT SET THIS MANUALLY) - small handle to the *headerDeadlineConn serving this stream; cancellation/idle-timeout owners set a past read deadline on it to unblock a parked body read WITHOUT firing fasthttp's pool-releasing close from a third goroutine (read-after-release). Nil on every non-passthrough stream, which keeps the legacy close-based path.
	BifrostContextKeyShutdownDone                        BifrostContextKey = "shutdown_done"                  // <-chan struct{} (set by isolatedPassthroughContext) - the instance ctx's Done() captured before WithoutCancel strips it, so a nil-ctx passthrough stream still tears down on Shutdown()
...
type BifrostErrorExtraFields struct {
	...
	// PassthroughResponse is an internal streaming-failover carrier (never serialized):
	// a passthrough stream reports its status as a data chunk, not a *BifrostError, so
	// the synthesized first-chunk error stashes the chunk's upstream response here for
	// the failover loop to classify and read Retry-After. Nil on all other errors.
	PassthroughResponse *BifrostPassthroughResponse `json:"-"`
