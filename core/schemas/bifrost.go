...
const (
	...
	BifrostContextKeyPassthroughCooledKeyIDs             BifrostContextKey = "bifrost-passthrough-cooled-key-ids"     // []string (key IDs temporarily avoided by passthrough failover after a 401/403/429 from a pool account; subtracted from the rotating key pool during selection so the next same-provider retry picks a different account (set by bifrost passthrough failover - DO NOT SET THIS MANUALLY))
