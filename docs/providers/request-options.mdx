...
| `-` (passthrough request field)            | `x-bf-fallbacks`                                     | `string`              | Comma-separated `provider/model` fallback ladder for passthrough routes, cheapest first. A transient failure retries the same provider within its budget before advancing                |
