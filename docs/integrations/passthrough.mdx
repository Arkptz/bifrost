...
Pass an `x-bf-fallbacks` header to give a passthrough request a fallback ladder. The value is
a comma-separated list of `provider/model` pairs, ordered cheapest first:

```bash
curl -X POST http://localhost:8080/anthropic_passthrough/v1/messages \
  -H "Content-Type: application/json" \
  -H "x-bf-fallbacks: anthropic/claude-3-5-haiku-latest,openai/gpt-4o-mini" \
  -d '{
...
Failover is cost-aware, so a transient problem never promotes a request to a pricier provider
on its first failure:

- A transient upstream failure (5xx, or a connection error) retries the **same** provider
  within a bounded budget before the ladder advances.
- A pool-account symptom (401, 402, 403) retries the same provider on a different key from
  its pool and never escalates — a rejected, unfunded, or blocked credential is not a reason
  to pay a more expensive provider.
- A rate limit (429) likewise retries the same provider, honoring `Retry-After`, and never
  escalates on its own — an account-level limit is usually shared across that provider's keys,
  so paying a pricier provider would not clear it.
- A terminal, request-bound error (for example 400) is returned verbatim without touching the
  ladder, because retrying an invalid request cannot help.
- The total number of upstream attempts across the whole ladder is bounded, so a long
  fallback list cannot fan out indefinitely.

The response body is forwarded byte-for-byte on every leg, so provider-specific fields such as
Anthropic `thinking` blocks and image payloads survive a fallback unchanged. Without the
header, behavior is identical to a plain passthrough request: one upstream call, no retries.
